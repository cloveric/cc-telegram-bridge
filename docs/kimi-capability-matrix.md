...
compatibility baseline is 0.42.0, where new/load, Search MCP, questions,
cancellation and reuse, and background Agent completion were live-verified.
The 0.41 permission and detached-question semantics remain in force. Remaining
gaps must still be re-probed before removal.

| Capability | Codex | Claude Code | Kimi Code alignment |
|---|---|---|---|
| Runtime transport | Persistent app-server by default; process fallback | Persistent stream-json worker | **Aligned:** persistent `kimi acp` worker with JSON-RPC framing; Kimi 0.42.0's default `agent-core-v2` path is live-verified |
...
| Full-auto/bypass | Native sandbox/full-auto and bypass modes | Native permission modes | **Aligned with explicit boundaries:** full-auto -> ACP `yolo` (ordinary tools auto-approved; sensitive commands may still ask) and is the Kimi default when no bridge mode is stored. Pre-0.41 bypass values without the new acknowledgement also resolve to `yolo`; a fresh explicit unsafe command records the acknowledgement and selects ACP `auto`, which is true Never Ask in Kimi 0.41.0 and executes dangerous or unanalyzable commands without interruption. Neither is an OS sandbox |
| Structured questions | Native request-user-input path | Native `AskUserQuestion` | **Partially aligned:** ACP-advertised single-choice options render in both Lark and Telegram. Kimi 0.41.0 background questions may outlive the foreground turn; TaroCub retains their approval lifecycle, routes late requests through Hook-recorded question tasks, and fails closed on worker teardown or expiry. Free-text and verified multi-question input remain unavailable |
...
| MCP servers | Native Codex MCP configuration | Native Claude MCP/plugins | **Aligned on Kimi 0.42.0:** native Kimi user/project MCP/plugins remain active, and TaroCub injects its complete configured MCP list, including the Brave/Tavily stdio Search MCP, through ACP for every new or loaded session. Structured MCP results retain object content, and session initialization fails closed instead of silently omitting search capability |
