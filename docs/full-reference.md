...
- native progress cards for long-running turns, including Claude child-agent text attached to the matching parent tool panel, plus queue-wait cards with a stop button. Claude MCP startup errors are sanitized and shown in the card/final result instead of silently disabling tools. Same-conversation messages remain FIFO by default; optional `CCTB_LARK_QUEUE_MODE=preempt|batch|preempt-batch` enables preempt/batch behavior only when explicitly configured;
...
4. BotFather will reply with a **bot token**; this guide represents it as `<bot-token-from-BotFather>`
